...
class nnAgent(Agent):
    def __init__(self, obs_space, device):
        Agent.__init__(self)
        self.device = device
        self.net = Model(obs_space).to(device)
    ...
    def train(self):
        print("training policy gradient net...")
        R = 0
        rewards_ = torch.zeros(len(self.rewards))
        for t in reversed(range(len(self.rewards))):
            R = self.rewards[t] + self.gamma * R
            rewards_[t] = R

        probs = torch.stack(self.probs)
        rewards_ = torch.tensor(rewards_).to(self.device)

        self.optimizer.zero_grad()
        loss = -(torch.log(probs) * rewards_).sum()
        print(f"loss = {loss}")
